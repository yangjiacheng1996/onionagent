...
我设计的工作区结构如下，已经经过严格论证，所以请完全遵循如下设计。
```
workspace/
    ...
    |—— PLAN.jsonl            计划看板
    |—— tools.jsonl           工具列表
    |—— memory/YYYYMMDD.md    每日记忆（grep加载，不超过10000 Token）
...
1. 如果这个工作区目录为空。直接初始化。
2. 如果这个工作区目录内已有内容。测试provider.toml中的模型（采用openai库，不采用requests库）。尝试加载session.jsonl为json list。工作区中其他文件和目录只做存活检查，保证有文件或目录即可，不做就绪检查（检查其中内容）。
