...
你提出的问题非常好，我已经想到了，所以在我的项目中，会有src/infrastructure/tool_channel这个模块。tool_list.py是将buildin、mcp、skills三种工具汇总成一份tool_list传给openai_tool_engine，大模型发出的工具指令，由tool_router.py解析并路由调用对应的client。所以会建立一套路由规则。但不是现在要考虑的事情