...
# - PowerShell 7 / Windows Terminal:中文正常显示
# - PowerShell 5.1 + chcp 65001:中文正常显示
# - 重定向到文件:UTF-8 BOM-less,跨平台可读
# 失败时静默(老 Python 3.6 之前没有 reconfigure),保持向后兼容。
...
class ProgressiveDisclosureEngine:
    ...
    def __init__(self, skills_root: Path, max_cache_size: int = 100, script_timeout: int = 60):
        """初始化渐进式披露引擎。
        
        参数:
            skills_root: Skills 根目录路径
            max_cache_size: LRU 缓存的最大大小
            script_timeout: 脚本执行默认超时(秒)
        """
    ...
    def load_reference(self, skill_name: str, ref_name: str) -> str:
        ...
        查找顺序(由近及远):
          1. LRU 缓存
          2. references/<ref_name>.md    (官方规范)
          3. <ref_name>.md               (Skill 根目录,一些老 skill 这样放)
          4. <ref_name>(任意扩展名)       (兜底,按文件名精确匹配)
        
        参数:
            skill_name: 技能名称
            ref_name: 参考文档名称（不含 .md 扩展名,或带扩展名均可）
        ...
        # 1. 已扫描的 references 字典(以 .md 扩展名归一化)
        ref_key = ref_name if ref_name.endswith(".md") else ref_name + ".md"
        stem_key = ref_name[:-3] if ref_name.endswith(".md") else ref_name
        for key in (ref_key, stem_key, ref_name):
            if key in skill.references:
                content = skill.references[key]
                self._cache_resource(cache_key, content)
                return content
        
        # 2. 优先在 references/ 子目录找
        candidates = [
            skill.path / "references" / f"{stem_key}.md",
            skill.path / "references" / ref_name,
        ]
        # 3. fallback 到 skill 根目录(一些老 skill 把 reference 放在这)
        candidates.extend([
            skill.path / f"{stem_key}.md",
            skill.path / ref_name,
        ])
        ...
        raise SkillError(
            f"Reference not found: '{ref_name}' in skill '{skill_name}'. "
    ...
    def execute_script(self, skill_name: str, script_name: str, *args, timeout: Optional[int] = None) -> tuple[int, str, str]:
        """执行技能脚本。
        ...
        跨平台执行策略:
            1. 若脚本有 shebang(#!/usr/bin/env python 等),直接当可执行跑
            2. 否则按扩展名匹配解释器:.py → python,.sh → bash,.js → node
            3. Windows 上 .sh 用 bash(Git Bash);缺失则报明确错误
        """
        skill = self.load_skill_instruction(skill_name)
        ...
        # 决定解释器
        cmd = _build_script_command(script_path, list(args))
        ...
        try:
            ...
            raise SkillError(f"Script execution timed out after {effective_timeout}s: {script_name}")
        except FileNotFoundError as e:
            raise SkillError(f"Interpreter not found for {script_name}: {e}")
    ...
    def get_skill_prompt(self, skill_name: str, level: DisclosureLevel = DisclosureLevel.L2_INSTRUCTION) -> str:
        ...
        注意:
            这是渐进式披露的"全文 dump"版本,只用于 L1/L2 和调试;
            L3 摘要模式走 cli_disclosure 的 _disclosure_l3_summary() 路径,
            避免一次性把 references 全文塞进 context。
        """
...
def _build_script_command(script_path: Path, args: list[str]) -> list[str]:
    """根据 shebang / 扩展名 / 平台构造跨平台执行命令。

    跨平台策略:
      - POSIX (Linux/macOS):有 shebang 直接当可执行;否则按扩展名匹配
      - Windows:没原生 shebang 机制,按扩展名强制匹配解释器(.py/.sh/.js);
        .exe/.bat/.cmd 等原生可执行直接跑
      - 都缺解释器时退回到 str(script_path),让 subprocess 报明确错误

    返回一个可以直接 subprocess.run() 的 argv 列表。
    """
    suffix = script_path.suffix.lower()
    is_windows = sys.platform == "win32"

    # Windows 原生可执行(无需解释器)
    if is_windows and suffix in (".exe", ".bat", ".cmd", ".com", ".ps1"):
        if suffix == ".ps1":
            pwsh = shutil.which("pwsh") or shutil.which("powershell")
            if pwsh is None:
                return [str(script_path)] + args
            return [pwsh, "-File", str(script_path)] + args
        return [str(script_path)] + args

    # POSIX:有 shebang 直接当可执行
    if not is_windows:
        try:
...
def cli_to_json(skill_path: Path) -> int:
...
def cli_to_xml(skill_paths: list[Path]) -> int:
...
def cli_disclosure(skill_name: str, skills_root: Path, level: str = "L2", full: bool = False) -> int:
    """展示技能内容的渐进式披露。

    L1: 元数据(单行,始终加载)
    L2: SKILL.md 正文(完整 markdown,技能被激活时加载)
    L3: 摘要模式(只列 references/scripts/assets 清单 + 字符数,不输出全文)
        --full 时切换到全文模式(L3 资源全部 dump 出来,调试用)

    L3 摘要 vs 全文的取舍:
      - 摘要模式:符合 Anthropic 渐进式披露精神,LLM 看完摘要再决定调 read-ref
      - 全文模式:可能一次输出几 MB token,慎用(本地调试或离线分析场景)

    参数:
        skill_name: 技能名称
        skills_root: Skills 根目录
        level: 披露层级 (L1/L2/L3)
        full: 仅对 L3 生效;True 输出全文,False(默认)只输出摘要
    ...
    try:
        ...
        if level == "L1":
            ...
            if full:
                ...
                return _disclosure_l3_summary(engine, skill_name)
        else:
...
def _disclosure_l3_summary(engine: "ProgressiveDisclosureEngine", skill_name: str) -> int:
    """disclosure L3 的摘要模式:不输出 reference 全文,只列清单。

    输出格式:
        # <skill-name> - L3 资源摘要
        # SKILL.md body: <N> chars

        ## references/  (3 项)
          - FORMS.md  (1.2K chars)
          - REFERENCE.md  (8.4K chars)
          - extra.md  (200 chars)
        ...
        ## scripts/  (5 项)
          - extract.py  (executable: True)
          ...

        # 真正想读 reference 全文?用 read-ref 命令
        # 想批量 dump L3 全部内容?用 disclosure --level L3 --full
    """
    skill = engine.load_skill_instruction(skill_name)

    lines: list[str] = []
    lines.append(f"# {skill.properties.name} - L3 资源摘要")
    lines.append(f"# 位置: {skill.path}")
    lines.append(f"# SKILL.md body: {len(skill.body)} chars")
    lines.append("")

    # references/(子目录)
    if skill.references:
        lines.append(f"## references/  ({len(skill.references)} 项)")
        for name in sorted(skill.references.keys()):
            size = len(skill.references[name])
            lines.append(f"  - {name}.md  ({_human_size(size)})")
        lines.append("")
    ...
    for item in skill.path.iterdir():
        ...
        try:
            ...
            root_mds.append((item.name, len(content)))
        except OSError:
            root_mds.append((item.name, 0))
    if root_mds:
        lines.append("## <root>/*.md  ({0} 项,兼容读取)".format(len(root_mds)))
        for name, size in sorted(root_mds):
            lines.append(f"  - {name}  ({_human_size(size)})")
        lines.append("")

    # scripts/
    if skill.scripts:
        lines.append(f"## scripts/  ({len(skill.scripts)} 项)")
        for name in sorted(skill.scripts.keys()):
            p = skill.scripts[name]
            size = p.stat().st_size if p.exists() else 0
            lines.append(f"  - {name}  ({_human_size(size)}, executable: {os.access(p, os.X_OK)})")
        lines.append("")

    # assets/
    if skill.assets:
        lines.append(f"## assets/  ({len(skill.assets)} 项)")
        for rel in sorted(skill.assets.keys()):
            p = skill.assets[rel]
            size = p.stat().st_size if p.exists() else 0
            lines.append(f"  - {rel}  ({_human_size(size)})")
        lines.append("")

    lines.append("# 读 reference 全文 → read-ref <skill> <ref-name>")
    lines.append("# 批量 dump L3 全部内容 → disclosure --level L3 --full")
...
def cli_read_body(skill_path: Path) -> int:
    """读取 SKILL.md 的正文(L2 指令,不含 YAML frontmatter)。

    用于渐进式披露中"技能被激活后,把 SOP 指令塞进上下文"的环节。
    与 to-json 的区别:to-json 输出元数据 JSON,read-body 输出纯 markdown 文本。
...
def cli_list_resources(skill_name: str, skills_root: Path) -> int:
    """列出某个 Skill 的所有 L3 资源(references/scripts/assets)。

    按目录分组,每行一个条目:
      - references/  : 参考文档(只列 .md)
      - <root>/*.md  : 根目录散落的 .md(兼容老 skill 把 reference 放根目录的写法)
      - scripts/     : 可执行脚本
      - assets/      : 静态资源(相对路径)
    ...
    try:
        ...
        print(f"# {skill.properties.name} - L3 资源清单")
        print(f"# 位置: {skill.path}")
        print()

        # 找出根目录散落的 .md(非 SKILL.md / skill.md)
        root_mds = []
        for item in skill.path.iterdir():
            if not item.is_file():
                continue
            ...
            if item.name.lower() == "skill.md":
                continue
            root_mds.append(item.name)

        if not skill.references and not skill.scripts and not skill.assets and not root_mds:
            print("(该 Skill 没有可选资源目录)")
            return 0

        if skill.references:
            print(f"## references/  ({len(skill.references)} 项,均为 .md)")
            for name in sorted(skill.references.keys()):
                print(f"  - {name}.md")
            print()

        if root_mds:
            print(f"## <root>/*.md  ({len(root_mds)} 项,根目录散落)")
            for name in sorted(root_mds):
                print(f"  - {name}    (兼容读取,可用 read-ref 直接读)")
            print()

        if skill.scripts:
            print(f"## scripts/  ({len(skill.scripts)} 项)")
            for name in sorted(skill.scripts.keys()):
                p = skill.scripts[name]
                print(f"  - {name}    (executable: {os.access(p, os.X_OK)})")
            print()

        if skill.assets:
            print(f"## assets/  ({len(skill.assets)} 项)")
            for rel in sorted(skill.assets.keys()):
                p = skill.assets[rel]
                size = p.stat().st_size if p.exists() else 0
...
def cli_read_reference(skill_name: str, ref_name: str, skills_root: Path) -> int:
    """读取 references/ 中的某个参考文档(纯文本,完整内容)。

    L3 加载。LLM 真正用到某 reference 时才调用,避免一次性塞爆上下文。
    ...
    try:
        engine = ProgressiveDisclosureEngine(skills_root)
        content = engine.load_reference(skill_name, ref_name)
        print(content)
        return 0
    except SkillError as e:
        print(f"Error: {e}", file=sys.stderr)
...
def cli_run_script(skill_name: str, script_name: str, skills_root: Path, script_args: list[str], timeout: int = 60) -> int:
    """执行 scripts/ 中的某个脚本。

    行为细节:
      - 第一行是 shebang 走解释器;否则按 Windows 关联或 python 兜底
      - 超时默认 60s(可用 --timeout 改)
      - 退出码 = 脚本的 returncode;非 0 走 stderr 输出但仍按 0 退出 CLI
        (因为脚本本身的非 0 退出是"业务失败",不是 CLI 故障)
      - 脚本不存在时,自动列出该 skill 的所有 scripts 供选择(修复导向)

    退出码:
        0: 脚本执行成功(returncode == 0)
        1: 脚本不存在或超时
        2: 脚本执行但 returncode != 0(业务失败)
    """
    try:
        engine = ProgressiveDisclosureEngine(skills_root)
        # 临时覆盖 engine 的 timeout
        original_timeout = engine._script_timeout if hasattr(engine, "_script_timeout") else 60
        engine._script_timeout = timeout
        try:
            returncode, stdout, stderr = engine.execute_script(skill_name, script_name, *script_args)
        finally:
            engine._script_timeout = original_timeout
        ...
        msg = str(e)
        # 修复导向:脚本不存在时,列出该 skill 的所有 scripts
        if "Script not found" in msg:
            try:
                hint_engine = ProgressiveDisclosureEngine(skills_root)
                hint_skill = hint_engine.load_skill_instruction(skill_name)
                if hint_skill.scripts:
                    msg += f"\n  Available scripts in '{skill_name}':"
                    for s in sorted(hint_skill.scripts.keys()):
                        msg += f"\n    - {s}"
                    msg += f"\n  Tip: 确认脚本名是否拼写错误,或者 '没传文件参数给脚本' 也会报此错(脚本要求的参数列表可看 SKILL.md 或脚本 --help)"
            except Exception:
                pass
...
命令:
  ...
  run <skill> <script>           执行 scripts/ 中的某个脚本(-- 分隔符后透传脚本参数)
...
示例:
  ...
  python agent_skills_client.py run pdf extract_text.py --root ./skills -- input.pdf
  python agent_skills_client.py run pdf check_fillable_fields.py --root ./skills --timeout 10 -- form.pdf
""")
...
def main():
    ...
    if command == "validate":
        ...
        sys.exit(cli_to_xml(paths))

    elif command == "read-body":
        if len(sys.argv) < 3:
            print("错误: 需要指定 Skill 路径", file=sys.stderr)
            sys.exit(1)
        path = Path(sys.argv[2])
        sys.exit(cli_read_body(path))
        ...
        while i < len(sys.argv):
            ...
            if arg == "--root" or arg == "-r":
                ...
                print("用法: python agent_skills_client.py disclosure <skill-name> [--root <path>] [--level L1|L2|L3] [--full]")
                print("  --full  仅对 L3 生效;默认 L3 只输出资源摘要,加 --full 才输出全文")
                sys.exit(0)
            else:
                skill_name = arg
                i += 1
        ...
        sys.exit(cli_disclosure(skill_name, skills_root, level, full))

    elif command in ("list-resources", "ls"):
        # list-resources 共享 disclosure 的 --root 选项解析逻辑
        skill_name = None
        skills_root = Path(".")
        i = 2
        while i < len(sys.argv):
        ...
        if not skill_name:
            print("错误: 需要指定技能名称", file=sys.stderr)
            sys.exit(1)
        sys.exit(cli_list_resources(skill_name, skills_root))

    elif command in ("read-ref", "cat-ref"):
        # 用法: read-ref <skill> <ref-name> [--root <path>]
        if len(sys.argv) < 4:
            print("用法: read-ref <skill-name> <ref-name> [--root <path>]", file=sys.stderr)
            sys.exit(1)
        skill_name = sys.argv[2]
        ref_name = sys.argv[3]
        skills_root = Path(".")
        if len(sys.argv) >= 6 and sys.argv[4] in ("--root", "-r"):
            skills_root = Path(sys.argv[5])
        sys.exit(cli_read_reference(skill_name, ref_name, skills_root))

    elif command == "run":
        # 用法: run <skill> <script> [--root <path>] [--timeout <sec>] -- <args...>
        #   -- 之前是外层选项,-- 之后全部透传给脚本(避免与脚本自己的 --root 等冲突)
        #   --root 是 --skills-root 的短别名,--timeout 是 --script-timeout 的短别名
        if len(sys.argv) < 4:
            print(
                "用法: run <skill-name> <script-name> [--root <path>] [--timeout <sec>] -- <args...>",
                file=sys.stderr,
            )
            print(
                "  注: '--' 之前是外层选项,'--' 之后全部透传给脚本(标准 POSIX 风格)",
                file=sys.stderr,
            )
            sys.exit(1)
        skill_name = sys.argv[2]
        ...
        # 定位 '--' 分隔符(从位置 4 开始找,因为前 3 个是 run/skill/script)
        try:
            sep_idx = sys.argv.index("--", 4)
        except ValueError:
        ...
        while i < sep_idx:
            arg = sys.argv[i]
            if arg in ("--root", "-r", "--skills-root") and i + 1 < sep_idx:
                skills_root = Path(sys.argv[i + 1])
                i += 2
            elif arg in ("--timeout", "--script-timeout") and i + 1 < sep_idx:
                try:
                    timeout = int(sys.argv[i + 1])
                except ValueError:
                    print(
                        f"错误: --timeout 需要整数,得到 '{sys.argv[i + 1]}'",
                        file=sys.stderr,
                    )
                    sys.exit(1)
                i += 2
                ...
                print(
                    "  示例: run pdf check_fillable_fields.py --root C:\\skills --timeout 10 -- form.pdf",
                    file=sys.stderr,
                )
                print(
                    "  '--' 之前是外层选项;'--' 之后全部透传给脚本(避免与脚本自己的 --root 等冲突)",
        ...
        # -- 之后的所有参数原样透传给脚本
        if sep_idx < len(sys.argv):
            script_args.extend(sys.argv[sep_idx + 1:])

        sys.exit(cli_run_script(skill_name, script_name, skills_root, script_args, timeout))
